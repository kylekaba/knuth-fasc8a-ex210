...
struct CheckpointHeader{char magic[8];uint32_t version,order,steps,chunk,count;uint64_t seed,matrix_hash,eigen_hash;};
#pragma pack(pop)
static void savecheckpoints(const std::string&fn,const Header&h,uint32_t chunk,const std::vector<std::vector<E>>&checkpoints){CheckpointHeader ch{};memcpy(ch.magic,"KRC101V1",8);ch.version=1;ch.order=h.n;ch.steps=h.bm_terms;ch.chunk=chunk;ch.count=checkpoints.size();ch.seed=h.seed;ch.matrix_hash=h.matrix_hash;ch.eigen_hash=h.eigen_hash;std::ofstream out(fn,std::ios::binary);if(!out)throw std::runtime_error("write "+fn);out.write((char*)&ch,sizeof ch);for(const auto&v:checkpoints){if(v.size()!=h.n)throw std::runtime_error("checkpoint size");out.write((char*)v.data(),2*v.size());}if(!out)throw std::runtime_error("write checkpoint data");}
int main(int ac,char**av){for(int a=1;a<101;a++)for(int b=1;b<101;b++)if(a*b%101==1){invp[a]=b;break;}if(ac<4){std::cerr<<"usage MATRIX CERT EIG_OR_DASH [CHECKPOINTS]\n";return 1;}std::string mfn=av[1],cfn=av[2],efn=av[3],checkpointfn=ac>4?av[4]:"";Mat A=loadmat(mfn);std::ifstream f(cfn,std::ios::binary);if(!f)throw std::runtime_error("open cert");Header h{};f.read((char*)&h,sizeof h);if(std::string(h.magic,8)!="KMW2CERT"||h.version!=2||h.prime!=101||h.nonresidue!=2||h.lambda!=50)throw std::runtime_error("bad cert header");if(h.matrix_hash!=hashfile(mfn))throw std::runtime_error("matrix hash mismatch");Op op;op.A=std::move(A);op.border=h.border;if(op.border){if(efn=="-")throw std::runtime_error("eigenvector required");op.eig=loadeig(efn);if(op.eig.n!=op.A.n||op.eig.pivot!=h.pivot||op.eig.v[op.eig.pivot]==0||h.eigen_hash!=hashfile(efn))throw std::runtime_error("eigenvector metadata/hash");std::vector<E>x(op.A.n),y(op.A.n);for(uint32_t i=0;i<op.A.n;i++)x[i]={op.eig.v[i],0};Op q=op;q.border=false;q.apply(x,y);for(E z:y)if(!zero(z))throw std::runtime_error("eigenvector residual nonzero");}else if(efn!="-")throw std::runtime_error("use - for normal cert");op.n=op.A.n+op.border;if(h.n!=op.n||h.terms!=h.bm_terms+32||h.bm_terms!=2*h.n)throw std::runtime_error("dimension/term mismatch");std::vector<E>stored(h.terms),storedC(h.degree+1);f.read((char*)stored.data(),2*stored.size());f.read((char*)storedC.data(),2*storedC.size());char extra;if(f.read(&extra,1))throw std::runtime_error("trailing certificate bytes");if(!f.eof()&&!f)throw std::runtime_error("truncated cert");
 uint32_t N=op.n,checkpointChunk=1024;std::vector<E>dR(N),dL(N),u(N),x(N),dx(N),y(N),seq(h.terms);uint64_t rng=h.seed;for(uint32_t i=0;i<N;i++){do{dR[i]={(uint8_t)(sm64(rng)%101),(uint8_t)(sm64(rng)%101)};}while(zero(dR[i]));do{dL[i]={(uint8_t)(sm64(rng)%101),(uint8_t)(sm64(rng)%101)};}while(zero(dL[i]));u[i]={(uint8_t)(sm64(rng)%101),(uint8_t)(sm64(rng)%101)};x[i]={(uint8_t)(sm64(rng)%101),(uint8_t)(sm64(rng)%101)};}std::vector<std::vector<E>>checkpoints;if(!checkpointfn.empty())checkpoints.push_back(x);
#ifdef _OPENMP
...
#pragma omp single
   {x.swap(y);if(!checkpointfn.empty()&&k+1<=h.bm_terms&&((k+1)%checkpointChunk==0||k+1==h.bm_terms))checkpoints.push_back(x); if((k+1)%5000==0) std::cerr<<"moments "<<(k+1)<<"/"<<h.terms<<"\n";}
  }
 }
#else
 for(uint32_t k=0;k<h.terms;k++){
  ...
  for(uint32_t i=0;i<N;i++)dx[i]=mul(dR[i],x[i]);op.apply(dx,y);for(uint32_t i=0;i<N;i++)y[i]=mul(dL[i],y[i]);x.swap(y);if(!checkpointfn.empty()&&k+1<=h.bm_terms&&((k+1)%checkpointChunk==0||k+1==h.bm_terms))checkpoints.push_back(x);if((k+1)%5000==0)std::cerr<<"moments "<<(k+1)<<"/"<<h.terms<<"\n";
 }
#endif
